...
class _DriversListState extends State<DriversList> {
  ...
  void fetchDrivers() async {
    ...
    try {
      ...
      for (var doc in driversSnapshot.docs) {
        ...
        DocumentSnapshot driverInfoSnapshot = await firestore
            .collection("drivers")
            .doc(driverId)
            .collection("info")
            .doc("driverInfo")
            .get();
        DocumentSnapshot busInfoSnapshot = await firestore
            .collection("drivers")
            .doc(driverId)
            .collection("info")
            .doc("busInfo")
        ...
        if (driverInfoSnapshot.exists && busInfoSnapshot.exists) {
          Map<String, dynamic> driverInfo =
              driverInfoSnapshot.data() as Map<String, dynamic>;
          Map<String, dynamic> busInfo =
              busInfoSnapshot.data() as Map<String, dynamic>;
  ...
  void filterDrivers(String query) {
    List<Map<String, dynamic>> temp = drivers.where((driver) {
      String firstName =
          driver["driverInfo"]["firstName"]?.toString().toLowerCase() ?? "";
      String lastName =
          driver["driverInfo"]["lastName"]?.toString().toLowerCase() ?? "";
      String busNumber =
          driver["busInfo"]["busNumber"]?.toString().toLowerCase() ?? "";
      String plateNumber =
          driver["busInfo"]["plateNumber"]?.toString().toLowerCase() ?? "";
        ...
        child: Column(
          ...
          children: [
            ...
            TextField(
              ...
              onChanged: (value) => filterDrivers(value),
            ),
            const SizedBox(height: 20),
            Expanded(
              child: ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  String firstName =
                      driver["driverInfo"]["firstName"] ?? "Unknown";
                  String lastName = driver["driverInfo"]["lastName"] ?? "";
                  String fullName = "$firstName $lastName".trim();
                  String busNumber =
                      driver["busInfo"]["plateNumber"] ?? "No Bus Info";
